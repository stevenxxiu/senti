...
#  
#  Description: Scores SemEval-2015 task 10, subtask A
#
...
use constant GOLD_FILE => 'SemEval2015-task10-test-A-gold.txt';
...
for (; <INPUT>; $lineNo++) {
	...
	die "Wrong file format: $_" if (!/^NA\t(T[0-9]+)\t([0-9]+)\t([0-9]+)\t(positive|negative|neutral)/);
	my ($propID, $pStart, $pEnd, $proposedLabel) = ($1, $2, $3, $4);

	### 1.2	. Check the gold file format
	#NA	T14114531	positive
	$_ = <GOLD>;
	die "Wrong file format!" if (!/^[0-9]+\t(T[0-9]+)\t([0-9]+)\t([0-9]+)\t(positive|negative|neutral)/);
	my ($dataset, $goldID, $tStart, $tEnd, $trueLabel) = ('Twitter2015', $1, $2, $3, $4);

	### 1.3. Check the spans
	die "$pStart--$pEnd, $tStart--$tEnd" if (($pStart != $tStart) || ($pEnd != $tEnd));

	### 1.4. Make sure IDs match
...
close(GOLD) or die;
$lineNo--;
die "Too few lines: $lineNo" if (3092 != $lineNo);
